...
public sealed class TransactionsController(ITransactionService transactionService) : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (categoryId <= 0)
        {
            ModelState.AddModelError(
                nameof(categoryId),
                "Kategori kimliği sıfırdan büyük olmalıdır.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        ValidateTransaction(request.TransactionDate, request.Amount);
    ...
    private void ValidateTransaction(DateOnly transactionDate, decimal amount)
    {
